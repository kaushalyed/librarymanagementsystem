		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$("#dData","#"+dtbl+"_2").click(function(){
					...
					var customRetVal = ret[0];
					tableName = dtbl.split('_');
					
					onCS = {};
					onCS = $($t).triggerHandler("jqGridDelRowClickSubmit", [rp_ge[tableName[1]], postdata]);
					if(onCS === undefined && $.isFunction( rp_ge[tableName[1]].onclickSubmit ) ) {
						onCS = rp_ge[tableName[1]].onclickSubmit.call($t, rp_ge[tableName[1]], postdata) || {};
					}
					ret = $($t).triggerHandler("jqGridDelRowBeforeSubmit", [postdata]);
					if(ret === undefined) {
						ret = [true,"",""];
					}
					if( ret[0] && $.isFunction(rp_ge[tableName[1]].beforeSubmit))  {
						ret = rp_ge[tableName[1]].beforeSubmit.call($t, postdata);
					}
					if(customRetVal && !rp_ge[tableName[1]].processing) {
						rp_ge[tableName[1]].processing = true;
						...
						if($t.p == undefined){
							opers = customParamObject;
						}else{
							opers = $t.p.prmNames;	
						}
						postd = $.extend({},rp_ge[tableName[1]].delData, onCS);
						oper = opers.oper;
						...
						for(pk in postdata) {
							if(postdata.hasOwnProperty(pk)) {
								postdata[pk] = $.jgrid.stripPref(($t.p == undefined) ? "" : $t.p.idPrefix, postdata[pk]);
							}
						}
						postd[idname] = postdata.join();
						$(this).addClass( commonstyle.active );
						var ajaxOptions = $.extend({
							url: rp_ge[tableName[1]].url || $($t).jqGrid('getGridParam','editurl'),
							type: rp_ge[tableName[1]].mtype,
							data: $.isFunction(rp_ge[tableName[1]].serializeDelData) ? rp_ge[tableName[1]].serializeDelData.call($t,postd) : postd,
							complete:function(data,status){
								var i;
								...
								if(data.status >= 300 && data.status !== 304) {
									...
									if ($.isFunction(rp_ge[tableName[1]].errorTextFormat)) {
										ret[1] = rp_ge[tableName[1]].errorTextFormat.call($t, data);
									}
									if(ret[1] === undefined) {
										ret[1] = status + " Status: '" + data.statusText + "'. Error code: " + data.status;
									}
									...
									if( ret[0] && $.isFunction(rp_ge[tableName[1]].afterSubmit) ) {
										ret = rp_ge[tableName[1]].afterSubmit.call($t, data, postd);
									}
								}
								if(ret[0] === false) {
									...
									if(rp_ge[tableName[1]].reloadAfterSubmit && $t.p.datatype !== "local") {
										$($t).trigger("reloadGrid");
									} else {
										if($t.p.treeGrid===true){
									...
									if($.isFunction(rp_ge[tableName[1]].afterComplete) || $._data( $($t)[0], 'events' ).hasOwnProperty('jqGridDelRowAfterComplete')) {
										var copydata = data;
										setTimeout(function(){
											$($t).triggerHandler("jqGridDelRowAfterComplete", [copydata, postd]);
											try {
												rp_ge[tableName[1]].afterComplete.call($t, copydata, postd);
											} catch(eacg) {
												// do nothing
											}
										},500);
								...
								rp_ge[tableName[1]].processing=false;
								if(ret[0]) {$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal, onClose: rp_ge[tableName[1]].onClose});}
							}
						}, $.jgrid.ajaxOptions, rp_ge[tableName[1]].ajaxDelOptions);

						if (!ajaxOptions.url && !rp_ge[tableName[1]].useDataProxy) {
							if ($.isFunction($t.p.dataProxy)) {
								rp_ge[tableName[1]].useDataProxy = true;
							} else {
								ret[0]=false;ret[1] += " "+$.jgrid.getRegional($t, 'errors.nourl');
							}
						}
						if (customRetVal) {
							if (rp_ge[tableName[1]].useDataProxy) {
								var dpret = $t.p.dataProxy.call($t, ajaxOptions, "del_"+tableName[1]); 
								if(dpret === undefined) {
									dpret = [true, ""];
								}
								if(dpret[0] === false ) {
									...
									$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal, onClose: rp_ge[tableName[1]].onClose});
								}
							}
							else {
								if(ajaxOptions.url === "clientArray") {
				...
				$("#eData", "#"+dtbl+"_2").click(function(){
					var cancelBtnTableName = dtbl.split('_');
					$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:rp_ge[cancelBtnTableName[1]].jqModal, onClose: rp_ge[cancelBtnTableName[1]].onClose});
					return false;
				});
				showFrm = $($t).triggerHandler("jqGridDelRowBeforeInitData", [$("#"+dtbl)]);
				if(showFrm === undefined) {
